import requests
import json
import getpass
import datetime
import time


def get_token(env):
    pw = getpass.getpass()
    token_payload = {
        "grant_type": "password",
        "username": "bjlipson@spscommerce.com",
        "client_id": 595,
        "password": pw
    }
    token_headers = {
        "Content-Type": "application/json;charset=UTF-8"
    }
    if env == "prod":
        token_url = f"https://id.spsc.io/identity/token/"
    else:
        token_url = f"https://{env}.id.spsc.io/identity/token/"
    r = requests.post(token_url, data=json.dumps(token_payload), headers=token_headers)
    return r.json()


def update():
    env = input("Environment: ")
    version = input("Version: ")
    res = get_token(env)

    with open("bdp.json") as data_file:
        data = json.load(data_file)
    headers = {
        "Authorization": "Bearer " + res["access_token"],
        "Content-Type": "application/json;charset=UTF-8"
    }
    id_key = f"{env}-id"
    if env == "prod":
        url = f"https://id.spsc.io/identity/v2/applications/{data[id_key]}/"
    else:
        url = f"https://{env}.id.spsc.io/identity/v2/applications/{data[id_key]}/"
    appData = requests.get(url, headers=headers)
    appData = appData.json()
    print(appData)
    payload = {
        "created_at": appData["created_at"],
        "id": appData["id"],
        "status": appData["status"],
        "in_maintenance_mode": appData["in_maintenance_mode"],
        "launchpad_display": appData["launchpad_display"],
        "name": appData["name"],
        "url": data["url"].replace("<ENVIRONMENT>", "dev").replace("<VERSION>", version),
        "description": appData["description"],
        "namespace": appData["namespace"],
        "language_list": appData["language_list"],
        "version": version,
        "is_archived": appData["is_archived"],
        "is_service": appData["is_service"],
        "is_public": appData["is_public"],
        "logo": appData["logo"],
        "slug_archive": appData["slug_archive"],
        "language_version": appData["language_version"],
        "maintenance_url": appData["maintenance_url"],
        "slug": appData["slug"],
        "permissions": data["permissions"],
        "publisher_org_id": appData["publisher_org_id"],
        "publisher_user_id": appData["publisher_user_id"],
        "roles": appData["roles"],
        "updated_at": datetime.datetime.fromtimestamp(time.time()).strftime("%Y-%m-%dT%H:%M:%SZ")
    }
    print(f"---------------------------------------URL---------------------------------------\n{url}")
    print(f"---------------------------------------DATA---------------------------------------\n{json.dumps(payload)}")
    r = json.loads(requests.put(url, data=json.dumps(payload), headers=headers).text)
    print(f"---------------------------------------RESPONSE---------------------------------------\n{r}")


def create():
    env = input("Environment: ")
    version = input("Version: ")
    res = get_token(env)
    with open("bdp.json") as data_file:
        data = json.load(data_file)
    data.pop("dev-id")
    data.pop("test-id")
    data.pop("stage-id")
    data.pop("prod-id")
    if "status" not in data:
        data["status"] = "Production"
    if "version" not in data:
        data["version"] = version
    data["url"] = data["url"].replace("<ENVIRONMENT>", "dev")
    data["url"] = data["url"].replace("<VERSION>", version)
    headers = {
        "Authorization": "Bearer " + res["access_token"],
        "Content-Type": "application/json;charset=UTF-8"
    }
    if env == "prod":
        url = f"https://id.spsc.io/identity/v2/applications/"
    else:
        url = f"https://{env}.id.spsc.io/identity/v2/applications/"
    print(f"---------------------------------------URL---------------------------------------\n{url}")
    print(f"---------------------------------------DATA---------------------------------------\n{json.dumps(data)}")
    r = json.loads(requests.post(url, data=json.dumps(data), headers=headers).text)
    print(f"---------------------------------------RESPONSE---------------------------------------\n{r}")
    if r["status"] != "error":
        with open("bdp.json", "r") as in_file:
            file_data = in_file.read()
        file_data = file_data.replace(f"<{env.upper()}ID>", r["id"])
        with open("bdp.json", "w") as out_file:
            out_file.write(file_data)
    else:
        print(f"---------------------------------------ERROR---------------------------------------\n{r['error_description']}")


create()
